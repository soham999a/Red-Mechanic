...
import { House, Cpu, MapPin, BarChart2, ClipboardList, Eye, EyeOff } from 'lucide-react'

const navItems = [
  { to: '/', label: 'Home', icon: House },
  { to: '/diagnostics', label: 'Diagnostics', icon: Cpu },
  { to: '/matching', label: 'Shop Matching', icon: MapPin },
  { to: '/dashboard', label: 'Dashboard', icon: BarChart2 },
  { to: '/history', label: 'Repair History', icon: ClipboardList },
]
    ...
    <aside className="fixed left-0 top-0 h-screen w-[240px] bg-navy flex flex-col z-50">
      ...
      <div className="px-4 py-4 border-t border-white/10">
        <div className="bg-white/5 rounded-lg p-3">
          <p className="text-white/60 text-[11px] font-semibold uppercase tracking-wider mb-2">Gemini API Key</p>
          ...
          <p className="text-white/30 text-[10px] mt-1.5">Your key never leaves your browser</p>
        </div>
      </div>
    </aside>
  )
